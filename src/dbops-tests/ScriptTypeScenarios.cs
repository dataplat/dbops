...
namespace DBOps.Tests
{
    ...
    public class ScriptTypeScenarios
    {
        ...
        DbUp.Engine.DatabaseUpgradeResult upgradeResult;
        DbUp.Engine.UpgradeEngine upgradeEngine;
        ...
        public ScriptTypeScenarios()
        {
            ...
            {
                new SqlScript("Script1.sql", "create table Foo (Id int identity)", new DbUp.Engine.SqlScriptOptions { ScriptType = ScriptType.RunOnce}),
                new SqlScript("Script2.sql", "alter table Foo add column Name varchar(255)", new DbUp.Engine.SqlScriptOptions { ScriptType = ScriptType.RunOnce}),
                new SqlScript("Script3.sql", "insert into Foo (Name) values ('test')", new DbUp.Engine.SqlScriptOptions { ScriptType = ScriptType.RunAlways})
            };
        ...
        public class TestScriptProvider : DbUp.Engine.IScriptProvider
        {
            ...
            public IEnumerable<DbUp.Engine.SqlScript> GetScripts(IConnectionManager connectionManager)
            {
                return sqlScripts;
            }
        }
